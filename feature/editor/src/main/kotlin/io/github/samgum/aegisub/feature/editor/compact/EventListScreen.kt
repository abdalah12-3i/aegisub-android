    ...
    AlertDialog(
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                OutlinedTextField(value = query, onValueChange = { query = it }, label = { Text("Query") }, singleLine = true)
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp), verticalAlignment = Alignment.CenterVertically) {
                    FilterChip(selected = target == ShiftTarget.BOTH, onClick = { target = ShiftTarget.BOTH }, label = { Text("Both") })
                    FilterChip(selected = target == ShiftTarget.START, onClick = { target = ShiftTarget.START }, label = { Text("Start") })
                    FilterChip(selected = target == ShiftTarget.END, onClick = { target = ShiftTarget.END }, label = { Text("End") })
                }
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Checkbox(checked = onlyAfter, onCheckedChange = { onlyAfter = it })
                    Text("Only selected & after")
                }
            }
        },
        confirmButton = {
...
private fun FramerateDialog(
    ...
    AlertDialog(
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                Text("From FPS", style = MaterialTheme.typography.labelMedium)
                Row(
                    modifier = Modifier.horizontalScroll(rememberScrollState()),
                    horizontalArrangement = Arrangement.spacedBy(6.dp),
                ) {
                ...
                Text("To FPS", style = MaterialTheme.typography.labelMedium)
                Row(
                    modifier = Modifier.horizontalScroll(rememberScrollState()),
                    horizontalArrangement = Arrangement.spacedBy(6.dp),
                ) {
...
private fun PropertiesSheet(
    ...
    ModalBottomSheet(onDismissRequest = onDismiss, sheetState = sheetState) {
        Column(
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                FilterChip(selected = collisions == "Normal", onClick = { collisions = "Normal" }, label = { Text("Normal") })
                FilterChip(selected = collisions == "Reverse", onClick = { collisions = "Reverse" }, label = { Text("Reverse") })
            }
            OutlinedTextField(
                value = timer,
